...
$Id: db2hbody.xsl,v 1.4 2002/05/29 19:28:31 eray Exp $
...
<xsl:template match="sect1/title | section/title">
  <h2 class="sect1">
    <xsl:call-template name="label-number"/>
    <xsl:text>. </xsl:text>
    <xsl:apply-templates/>
  </h2>
  <xsl:if test="$verbose = 1">
...
<xsl:template name="label">
  <xsl:choose>
    <xsl:when test="ancestor-or-self::copyrightpg">
...
Return string containing the full label-number of an element.
For example, the third chapter would be "3" and the seventh figure
inside that chapter would be "3-7".

-->

<xsl:template name="label-number">
  <xsl:choose>

    <xsl:when test="ancestor-or-self::figure">
      <xsl:call-template name="chaplevel"/>
      <xsl:text>-</xsl:text>
      <!-- LibXSLT doesn't handle the from attribute correctly... -->
      <xsl:number 
        value="count(preceding::figure) - 
	       count(ancestor-or-self::appendix/preceding::figure |
	             ancestor-or-self::chapter/preceding::figure |
		     ancestor-or-self::prefix/preceding::figure)+1"/>
    </xsl:when>
    ...
    <xsl:when test="ancestor-or-self::example">
      ...
      <!-- LibXSLT doesn't handle the from attribute correctly... -->
      <xsl:number 
        value="count(preceding::example) - 
	       count(ancestor-or-self::appendix/preceding::example |
	             ancestor-or-self::chapter/preceding::example |
		     ancestor-or-self::prefix/preceding::example)+1"/>
    </xsl:when>
    ...
    <xsl:when test="ancestor-or-self::table">
      ...
      <xsl:number 
        value="count(preceding::table) - 
	       count(ancestor-or-self::appendix/preceding::table |
	             ancestor-or-self::chapter/preceding::table |
		     ancestor-or-self::prefix/preceding::table)+1"/>
    </xsl:when>
    ...
    <xsl:when test="ancestor-or-self::refentry">
      ...
      <!-- LibXSLT doesn't handle the from attribute correctly... -->
      <xsl:number 
        value="count(preceding::refentry) - 
	       count(ancestor-or-self::appendix/preceding::refentry |
	             ancestor-or-self::chapter/preceding::refentry |
		     ancestor-or-self::prefix/preceding::refentry)+1"/>
    </xsl:when>
    ...
    <xsl:otherwise>
      ...
      <xsl:if test="ancestor-or-self::sect2">
        <xsl:text>.</xsl:text>
        <xsl:number 
          value="count(ancestor-or-self::sect2/preceding-sibling::sect2)+1"/>
      </xsl:if>
      <xsl:if test="ancestor-or-self::sect3">
        <xsl:text>.</xsl:text>
        <xsl:number 
          value="count(ancestor-or-self::sect3/preceding-sibling::sect3)+1"/>
      </xsl:if>
      <xsl:if test="ancestor-or-self::sect4">
        <xsl:text>.</xsl:text>
        <xsl:number 
          value="count(ancestor-or-self::sect4/preceding-sibling::sect4)+1"/>
      </xsl:if>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template name="filename">
  ...
  <xsl:choose>
    ...
    <xsl:when test="ancestor-or-self::partintro or
                    ...
                    ancestor-or-self::copyrightpg or
